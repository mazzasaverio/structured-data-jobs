...
class CareerCrawler:
    ...
    async def find_career_link(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                logfire.debug(
                    f"All links on page: {', '.join(found_links[:20])}{'...' if len(found_links) > 20 else ''}"
                )
    ...
    async def process_company(self, company: CompanyUrl, session: AsyncSession) -> None:
        ...
        try:
            ...
            # Extract content
            content_text = await convert_webpage_to_markdown(career_url)
            ...
            is_target, suggested_urls = json_data.get("is_target"), json_data.get(
                "suggested_urls"
            )
            ...
            if not is_target and suggested_urls:
                ...
                await self.explore_suggested_urls(
                    career_url,
                    suggested_urls,
                    company,
                    session,
            ...
            if is_target:
                logfire.info(
                    "Career page is a target (contains a list of job postings)",
                    url=career_url,
                )
            else:
    ...
    async def explore_suggested_urls(
        self,
        base_url,
        suggested_urls,
        company,
        ...
        for suggestion in suggested_urls:
            ...
            try:
                ...
                logfire.info(
                    f"Exploring suggested URL (depth {depth})", url=suggested_url
                )

                # Extract content
                suggestion_text = await convert_webpage_to_markdown(suggested_url)

                # Process content
                json_data = await process_content(
                    suggested_url, suggestion_text, llm_config
                )

                json_data["company_name"] = company.name
                json_data["company_id"] = company.id
                json_data["depth"] = depth
                json_data["parent_url"] = base_url
                ...
                if is_suggested_target:
                    logfire.info(
                        f"Found target page at depth {depth}", url=suggested_url
                    )
                    target_found = True
                    break
                ...
                # If not a target and we haven't reached max depth, explore further
                    ...
                    logfire.info(
                        f"Recursively exploring URLs from {suggested_url} (depth {depth+1})"
                    )
                ...
                logfire.error(
                    f"Error checking suggested URL",
                    url=suggested_url,
                    depth=depth,
                    error=str(e),
                )
    ...
    async def run(self) -> None:
        ...
        try:
            async with get_db_session() as session:
                ...
                for target_url in target_urls:
                    try:
                        ...
                        if not url_exists:
                            logfire.info(
                                f"Found broken target URL - deleting",
                                url=target_url.url,
                            )
                        ...
                        logfire.error(
                            f"Error checking target URL",
                            url=target_url.url,
                            error=str(e),
                        )

                # Subquery to find companies that have at least one VALID target URL
                companies_with_targets = (
                    select(FrontierUrl.company_id)
                ...
                logfire.info(
                    f"Found {len(companies)} companies to process (without targets or with broken targets)"
                )

                for company in companies:
                    try:
                        logfire.info(
                            f"Processing company: {company.name}", url=company.url
                        )
            ...
            logfire.error(
                f"Crawler error", error=str(e), traceback=traceback.format_exc()
            )
