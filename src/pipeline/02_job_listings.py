...
async def save_job_listings(job_listings: List[dict], frontier_url: FrontierUrl) -> None:
...
async def run_job_extractor():
    ...
    if not llm_config:
        logfire.error("Failed to load LLM configuration. Check if the 'job_extraction' key exists in config file.")
        return
        
    logfire.info("Loaded LLM configuration", model=llm_config.get("model", "unknown"))

    output_dir = Path(os.getenv('PROJECT_ROOT', '.')) / "data" / "job_listings"
    ...
    async with get_db_session() as session:
        ...
        for frontier_url in target_urls:
            ...
            job_id = frontier_url.id or "unknown"
            output_file_path = output_dir / f"job_listings_{domain}_{job_id}.json"  
            markdown_file_path = output_dir / f"job_listings_{domain}_{job_id}.md"

            content_text = await get_page_markdown(source_url, 
                                                 save_to_file=True,
                                                 filename=str(markdown_file_path))
            ...
            if not content_text:
                ...
                logfire.info(f"Successfully extracted content from URL", 
                           url=source_url, 
                           content_length=len(content_text))
                try:
                    json_data = await process_content(source_url, content_text, llm_config)
                except Exception as e:
                    logfire.error(f"Error processing content from URL", 
                                url=source_url, 
                                error=str(e),
            ...
            logfire.info(f"Saved job listings JSON data to file", file_path=str(output_file_path))
